...
public class Boj2143Arrsum {
	...
	private static int twoPointer(List<Integer> ls1,  Map<Integer, Integer> cntMap1, List<Integer> ls2,  Map<Integer, Integer> cntMap2, int target) {
		int len1 = ls1.size(); 
		int len2 = ls2.size(); 
		int p1 = 0;
		int p2 = len2-1;
		...
		while(p1<len1 && p2>=0) {
			...
			if(sum == target) {	// 같은 숫자 어떻게 처리??
				cnt += cntMap1.get(n1)*cntMap2.get(n2);
				p1++;
			}
